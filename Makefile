...
%.pdf : do_anyway
	@echo -e "\033[31mDvi produce\033[0m"
	$(DVI_PRODUCER) "$*.tex"
	...
	$(DVI_PRODUCER) "$*.tex"
	@echo -e "\033[31mPs produce\033[0m"
	$(PS_PRODUCER) "$*.dvi"
	@echo -e "\033[31mPDF produce\033[0m"
	$(PDF_PRODUCER) "$*.ps"
...
%.clean: do_anyway
	...
	@rm -f -- $*.aux $*.blg $*.glg $*.glo $*.gls $*.fax $*.idx $*.ilg $*.ind $*.ist $*.lof $*.log $*.loh $*.loi $*.lot $*.nav $*.out $*.snm $*.tns $*.toc $*.bbl $*.ps $*.dvi
